...
- [ ] T001 Add `pyproject.toml` in `components/sonavida/` for Python 3.12 managed with `uv`: package `sonavida` under `src/`, console entry point `sonavida`, dependencies `miraveja-persona` and `miraveja-studiolink` pinned to git commits, `httpx`, `pydantic` v2, `anyio`; dev group `pytest`, `ruff`, `mypy`
