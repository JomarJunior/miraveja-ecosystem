...
- **Dry run on the Studio**: `sonavida run --dry-run` pairs the real **🧠 ModelMora** and the real clock with the perception and gate stand-ins and the in-process reference stand-in. It is how the pre-alpha roster lives for real before **💬 DescriDiva**, **🧐 CuraGusta** and the Museum side exist; the gate stand-in stays bound to the reference stand-in, so this adds no bypass.
