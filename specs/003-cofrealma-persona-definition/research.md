...
- **Secrets**: the guard also blocks secret-shaped values in every scanned text file, as Constitution VIII requires of public repositories: private key blocks, access-token formats and credentials in URLs (the `SECRET_TOKENS` patterns shared with the `forbidden.secret` rule). It reports only file, line and "secret-shaped value". A broad `name = value` pattern is used for definitions only, since on code it would block ordinary lines such as `token: str`. A deliberate fake secret in a fixture is marked with `guard: fake-secret` on its line.
